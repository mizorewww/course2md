...
## 导航与信息减法复审（2026-09-06）

独立子代理复审确认上一轮验收漏掉了导航选中态：选中底色对侧栏仅约 1.01:1，虽然正文文字对比度充足，当前位置仍难辨认。以下约定替代上文关于常驻底栏和完整引擎列表的设计：

- 导航与文件夹共用选中样式：蓝底、白字、加粗，并提供可访问的选中语义；不再依靠几乎同亮度的浅色背景。
- 空闲时不渲染底栏。运行任务时才显示实际任务摘要及查看入口；移除与当前任务无关的识别配置草稿。保存失败在设置入口标记，详细错误仍在设置页。
- 默认引导只呈现「在本机识别」和「使用云端服务」。软件检测并选定适合本机的可用后端；本机选项说明音频留在电脑上，云端选项说明音频会发送到服务。
- CPU、GPU 等具体后端仅在高级设置中出现，不在默认引导列出 Python、驱动、llama-server 等技术要求。高级与简洁视图不同时呈现同一后端的两个选中项。
- 云端字段仅在选择云端服务时显示；仅字幕模式不显示引擎配置。
- 复审额外发现并修复了引导诊断入口被延迟导航覆盖、跳过引导后自动后端没有选中表现，以及删除底栏后保存失败反馈丢失的问题。

实机已验证首次默认本机选择、高级展开单选状态、诊断跳转、云端字段按需出现，以及离开设置页后「设置 · 未保存」仍然可见。原有桌面测试 10 项通过（2 项忽略），桌面 Clippy 通过；未添加脚手架测试。本轮未重新执行 Windows / Linux 实机测试。
